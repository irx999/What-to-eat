$(function () {
  ...
  function getRandomInt(min, max) {
    return (min = min || 100), (max = max || 0), Math.ceil(Math.random() * (min - max) + max);
  }
  
  function showComment(text) {
    var comment = $("<div class='comment'>" + text + "</div>");
    comment.on("animationend webkitAnimationEnd", function (event) {
      $(this).remove();
    }).appendTo($("body"));
  }
  ...
  function getCurrentOptions() {
    if (userType === "human" || userType === "company" || userType === "天街" || userType === "special") {
      if (currentTime === 1) { // 午饭
        return lunchOptions[userType];
      } else if (currentTime === 2) { // 晚饭
